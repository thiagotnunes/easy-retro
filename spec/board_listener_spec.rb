...
describe 'BoardListener' do
  ...
  it "should save published post its" do
    message = { "channel" => "/board", "data" => { "action" => "create", "postIt" => { "id" => "123_4", "text" => "some text" } } }

    @callback.should_receive(:call).with(message)
    @listener.outgoing message, @callback 

    @listener.postIts['123_4'].should eq message["data"]["postIt"] 
  end

  it "should return the current board when subscribing to a channel" do
    first_message = {"channel" => "/board", "data" => {"action" => "create", "postIt" => {"id" => "1", "text" => "some text"}}}
    @callback.should_receive(:call).with(first_message)
    @listener.outgoing first_message, @callback

    second_message = {"channel" => "/board", "data" => {"action" => "create", "postIt" => {"id" => "2", "text" => "more text"}}}
